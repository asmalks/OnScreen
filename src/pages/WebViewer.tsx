...
import { WebRTCHost, WebRTCViewer } from "@/lib/webrtc";

export default function WebViewer() {
  const { sessionId } = useParams();
  const isMobile = useIsMobile();
  const { localStream } = useSession();
              ...
              <div className="h-full w-full bg-black flex flex-col items-center justify-center relative overflow-hidden">
                ...
                {touchEnabled && (
                  <div className="absolute bottom-4 left-0 right-0 text-center z-20 pointer-events-none">
                    <span className="text-xs bg-foreground/10 px-3 py-1 rounded-full text-foreground/50 backdrop-blur-md">
                      Touch input active
                    </span>
                  </div>
                )}
